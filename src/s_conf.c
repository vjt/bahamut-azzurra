...
int attach_Iline(aClient *cptr, struct hostent *hp, char *sockhost)
{
	...
	if (aconf->flags & CONF_FLAGS_I_HOST_HAS_AT)
	{
	    uhost_has_at = 1;
	    ulen = ircsprintf(uhost, "%s@", cptr->username);
	    ircsprintf(uhost2, "%s@", cptr->username);
	}
	else
	{
	    uhost_has_at = 0;
	...
	strncat(uhost, sockhost, sizeof(uhost) - ulen);
        strncat(uhost2, cptr->hostip, sizeof(uhost2) - ulen);

	if ((match(aconf->host, uhost) == 0) ||
	    (match(aconf->host, uhost2) == 0))
	    return (attach_iline(cptr, aconf, uhost, uhost_has_at));
    }
...
#ifdef INET6
    ip6_expand(cptr->sockhost, sizeof(cptr->sockhost));
#endif
...
int rehash(aClient *cptr, aClient *sptr, int sig)
{
    ...
    int         i,  ret = 0, fd;
#ifdef ADMINSERV
    extern unsigned short as_bufflag;
    extern unsigned char *as_bufname;
#endif
    ...
    {
	if(sig == 0)
	    as_msg(sptr, AS_SUCCESS, "cleaned buffer for `%s'",
		    as_bufname);
	as_free_buf();
    }
#endif
	...
	else if(tmp2->status & (CONF_SQLINE|CONF_SGLINE))
	{
	    tmp = &tmp2->next;
	}
	else 
...
int oper_access[] =
{
    ~(OFLAG_ADMIN|OFLAG_SADMIN|OFLAG_ZLINE|OFLAG_ADMIN|OFLAG_SPAM), '*',
    OFLAG_LOCAL,   'o',
    OFLAG_GLOBAL,  'O',
    OFLAG_REHASH,  'r',
    OFLAG_DIE,     'D',
...
initconf(int opt, int fd)
{
	...
	if (aconf->status & CONF_CLIENT)
	{
		...
		{
		    char *tmpd = aconf->passwd;
		    char *tmp = aconf->passwd + 6;

		    aconf->flags |= CONF_FLAGS_I_HELPERPORT;
		    if(*tmp)
			tmp++;
		    DupString(aconf->passwd, tmp);
		    MyFree(tmpd);
		    ...
		    aConfList *List3)
{
    ...
    return find_conf_match_host(cptr->user->username, cptr->sockhost,
				List1, List2, List3);
}

/* This code in plain bahamut was in the find_conf_match function.
 * In the Azzurra patch we need to move it to this new function
 * to allow conf check to unregistered users. -INT
 */
